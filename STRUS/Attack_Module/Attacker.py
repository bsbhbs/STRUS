...
class Attacker():
    ...
    def get_target_label(self,uav_num,attack_type,aims=()):
        target_label1 = np.array([0] * uav_num)  # 是否被攻击
        target_label2 = np.array([0] * uav_num)  # 是否遭受下行攻击
        target_label3 = np.array([0] * uav_num)  # 是否遭受上行攻击
        for i in aims:
            target_label1[i] = 1
            if attack_type == 0:
                target_label2[i] = 1
    ...
    def attack_0(self,sample,attack_aims):
        ...
        for attack_aim in attack_aims:
            # 将攻击目标样本的指定特征列进行处理后重新赋值
            # sample: 样本数据
            # attack_aim: 攻击目标列名
            # [i for i in range(1,13)]: 特征列的索引范围，即第1到第12列
            # self.A_D.execute(): 执行数据处理操作，可能是对特征进行某种转换或攻击
            # 修改无人机传感器数据（索引1-12）
